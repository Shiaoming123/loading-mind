...
function providerSearchAction(query = "current agent search landscape sources") {
  return {
    action: "search",
    query,
    rationale: "Need source-backed evidence before writing the report.",
    expectedInformation: "Current facts, examples, comparisons, and implementation details."
  };
}

function providerChatResponse(options, {
  plannerActions = [providerSearchAction(), { action: "finish_search", rationale: "Enough source candidates for this test." }],
  reportContent = providerReportContent(),
  reportStatus = 200,
  reportError = ""
} = {}) {
  ...
  if (promptText.includes("LOADING_MIND_REACT_SEARCH_PLANNER")) {
    const action = plannerActions.shift() ?? { action: "finish_search", rationale: "Enough source candidates for this test." };
  ...
  if (reportStatus >= 400) {
    return new Response(JSON.stringify({ error: { message: reportError || "provider outage" } }), {
      status: reportStatus,
...
describe("agent runtime failure helpers", () => {
  ...
  it("records Tavily usage metadata during Live search", async () => {
    ...
    const plannerActions = [
      providerSearchAction("current agent search landscape sources"),
      { action: "finish_search", rationale: "Tavily returned enough source candidates for this test." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      if (urlText === "https://api.tavily.com/search") {
        tavilyQueries.push(JSON.parse(String(options?.body || "{}")).query);
        return new Response(JSON.stringify({
      ...
      if (urlText === "https://provider.example/v1/chat/completions") {
        return providerChatResponse(options, {
          plannerActions,
          reportContent: providerReportContent("结论：live provider 已基于来源包写出研究回答。建议按真实任务验证。")
        });
      }
    ...
    const finalReport = snapshot.events.find((event) => event.finalReport)?.finalReport;
    const decisionNode = snapshot.events
      .map((event) => event.graphEvent?.node)
      .find((node) => node?.id === "agent-search-decision-1" && node.status === "observed");
    const observationNode = snapshot.events
      .map((event) => event.graphEvent?.node)
      .find((node) => node?.id === "agent-search-observation-1");
    const completeNode = snapshot.events
      .map((event) => event.graphEvent?.node)
      .find((node) => node?.id === "agent-search-complete");

    expect(snapshot.run.runMode).toBe("live");
    expect(snapshot.run.status).toBe("completed");
    expect(tavilyQueries[0]).toBe("current agent search landscape sources");
    expect(decisionNode?.attributes).toMatchObject({
      planner: "llm_react",
      action: "search",
      query: "current agent search landscape sources"
    });
    expect(observationNode?.attributes.returnedSources).toBe("8");
    expect(completeNode?.attributes.finishReason).toBe("finish_search");
  ...
  it("falls back from Tavily to Brave Search during Live search", async () => {
    ...
    let braveCalls = 0;
    const plannerActions = [
      providerSearchAction("agent search landscape brave fallback first pass"),
      providerSearchAction("agent search landscape brave fallback second pass"),
      { action: "finish_search", rationale: "Brave fallback returned enough source candidates for this test." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      if (urlText === "https://api.tavily.com/search") {
      ...
      if (urlText === "https://provider.example/v1/chat/completions") {
        return providerChatResponse(options, {
          plannerActions,
          reportContent: providerReportContent("结论：Brave fallback 搜索后仍由 provider 写出研究回答。建议按同一任务集复测。")
        });
      }
      throw new Error(`Unexpected fetch ${urlText}`);
    };
  ...
  it("completes Live deep research when one search branch fails but enough sources remain", async () => {
    ...
    let searchCalls = 0;
    const plannerActions = [
      providerSearchAction("codex claude code comparison first pass"),
      providerSearchAction("codex claude code comparison second pass"),
      providerSearchAction("codex claude code comparison recovery pass"),
      { action: "finish_search", rationale: "Enough usable sources remain after one failed branch." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      if (urlText === "https://api.tavily.com/search") {
      ...
      if (urlText === "https://provider.example/v1/chat/completions") {
        return providerChatResponse(options, {
          plannerActions,
          reportContent: providerReportContent("结论：部分搜索分支失败后，provider 仍基于可用来源写出研究回答。建议标注风险并继续验证。")
        });
      }
      throw new Error(`Unexpected fetch ${urlText}`);
    };
  ...
  it("completes Live deep research when most fetches fail and uses a degraded fetch summary", async () => {
    ...
    process.env.TAVILY_API_KEY = "tvly-test";
    const plannerActions = [
      providerSearchAction("codex claude code comparison fetch degrade sources"),
      { action: "finish_search", rationale: "Tavily returned enough source candidates for this test." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      ...
      if (urlText === "https://provider.example/v1/chat/completions") {
        return providerChatResponse(options, {
          plannerActions,
          reportContent: providerReportContent("结论：部分抓取失败后，provider 使用可用摘要和正文写出研究回答。建议补抓关键来源。")
        });
      }
      throw new Error(`Unexpected fetch ${urlText}`);
    };
  ...
  it("falls back to deterministic report when provider output is not usable", async () => {
    ...
    process.env.TAVILY_API_KEY = "tvly-test";
    const plannerActions = [
      providerSearchAction("agent search landscape recovery sources"),
      { action: "finish_search", rationale: "Tavily returned enough source candidates for this test." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
  ...
  it("records a deterministic fallback when the provider fails", async () => {
    ...
    process.env.TAVILY_API_KEY = "tvly-test";
    const plannerActions = [
      providerSearchAction("agent search landscape provider failure sources"),
      { action: "finish_search", rationale: "Tavily returned enough source candidates for this test." }
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
  ...
  it("fails loud when the LLM search planner emits an unsupported action", async () => {
    const previousTavilyKey = process.env.TAVILY_API_KEY;
    process.env.TAVILY_API_KEY = "tvly-test";
    const plannerActions = [{ action: "browse", query: "unsupported tool" }];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      if (urlText === "https://provider.example/v1/chat/completions") {
        return providerChatResponse(options, { plannerActions });
      }
      throw new Error(`Unexpected fetch ${urlText}`);
    };
    ...
    expect(snapshot.run.status).toBe("failed");
    expect(snapshot.events.some((event) => event.error?.includes("unsupported action"))).toBe(true);
    expect(snapshot.events.find((event) => event.finalReport)?.finalReport?.kind).toBe("failure");
  });

  it("fails loud when LLM search reaches max rounds without enough usable sources", async () => {
    const previousTavilyKey = process.env.TAVILY_API_KEY;
    process.env.TAVILY_API_KEY = "tvly-test";
    let searchCalls = 0;
    const plannerActions = [
      providerSearchAction("thin source round one"),
      providerSearchAction("thin source round two"),
      providerSearchAction("thin source round three"),
      providerSearchAction("thin source round four")
    ];
    const fetchImpl = async (url, options) => {
      const urlText = String(url);
      if (urlText === "https://provider.example/v1/chat/completions") {
      ...
      if (urlText === "https://api.tavily.com/search") {
        searchCalls += 1;
        return new Response(JSON.stringify({
          request_id: `thin-${searchCalls}`,
          results: [{
            title: `Thin Source ${searchCalls}`,
            url: `https://example.com/thin-${searchCalls}`,
            content: `Thin snippet ${searchCalls}`,
            raw_content: `# Thin ${searchCalls}`
          }]
        }), { status: 200, headers: { "Content-Type": "application/json" } });
      }
      throw new Error(`Unexpected fetch ${urlText}`);
    ...
    expect(snapshot.events.some((event) => event.error?.includes("reached max rounds"))).toBe(true);
    expect(snapshot.events.find((event) => event.finalReport)?.finalReport?.kind).toBe("failure");
  });
